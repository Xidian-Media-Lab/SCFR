...
if __name__ == "__main__":
    ...
    for seq in seqlist:
        for qp in qplist:    
            ...
            original_seq= '../experiment/'+Model+'/Iframe_'+Iframe_format+'/dec/'+opt.testingdata_name+'_'+str(seq)+'_256x256_25_8bit_444_QP'+str(qp)+'.rgb'
            listR,listG,listB=RawReader_planar(original_seq, width, height,frames)

            # wtite ref and cur (rgb444) to file (yuv420)
            oriyuv_path='../experiment/'+Model+'/Iframe_'+Iframe_format+'/dec_yuv420/'+opt.testingdata_name+'_'+str(seq)+'_256x256_25_8bit_420_QP'+str(qp)+'.yuv'
