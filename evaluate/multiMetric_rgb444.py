...
if __name__ == "__main__":
    ...
    parser.add_argument("--testingdata_name", default='VOXCELEB', type=str, help="path to the input testing sequence")
    ...
    for seq in seqlist:
        ...
        for qp in qplist:
            ...
            f_org=open('../testing_sequence/'+opt.testingdata_name+'_'+str(seq)+'_'+str(width)+'x'+str(height)+'_25_8bit_444.rgb','rb')

            f_test=open('../experiment/'+Model+'/Iframe_'+Iframe_format+'/dec/'+opt.testingdata_name+'_'+str(seq)+'_256x256_25_8bit_444_QP'+str(qp)+'.rgb','rb')
            

            if metric['psnr']:
                f_psnr = open(result_dir+opt.testingdata_name+'_'+str(seq)+'_QP'+str(qp)+'_psnr.txt','w')
                sum_PSNR=0
            if metric['ssim']:
                f_ssim = open(result_dir+opt.testingdata_name+'_'+str(seq)+'_QP'+str(qp)+'_ssim.txt','w')
                sum_SSIM=0
            if metric['lpips']:
                f_lpips = open(result_dir+opt.testingdata_name+'_'+str(seq)+'_QP'+str(qp)+'_lpips.txt','w')
                sum_LPIPS=0
            if metric['dists']:    
                f_dists = open(result_dir+opt.testingdata_name+'_'+str(seq)+'_QP'+str(qp)+'_dists.txt','w')
                sum_DISTS=0
            ...
            print(opt.testingdata_name+'_'+str(seq)+'_QP'+str(qp)+'.rgb',"success. Time is %.4f"%(end-start))
            qpIdx+=1
        seqIdx+=1
    ...
    if metric['lpips']:
        np.savetxt(result_dir+opt.testingdata_name+'_result_lpips.txt', totalResult_LPIPS, fmt = '%.5f')
    if metric['dists']:
        np.savetxt(result_dir+opt.testingdata_name+'_result_dists.txt', totalResult_DISTS, fmt = '%.5f')
