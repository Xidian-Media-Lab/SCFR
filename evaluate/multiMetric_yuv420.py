...
if __name__ == "__main__":
    ...
    parser.add_argument("--testingdata_name", default='CFVQA', type=str, help="path to the input testing sequence")
 ...
 # 'CFVQA' OR 'VOXCELEB'
    if opt.testingdata_name=='CFVQA':
        frames=125
    if opt.testingdata_name=='VOXCELEB':
        frames=250        
    ...
    for seq in seqlist:
        ...
        for qp in qplist:
            ...
            original_video = '../testing_sequence/OriYuv/'+opt.testingdata_name+'_'+str(seq)+'_'+str(width)+'x'+str(height)+'_25_8bit_420.yuv'
                        
            encoded_video = '../experiment/'+Model+'/Iframe_'+Iframe_format+'/dec_yuv420/'+opt.testingdata_name+'_'+str(seq)+'_256x256_25_8bit_420_QP'+str(qp)+'.yuv'
            ...
            print(opt.testingdata_name+'_'+str(seq)+'_QP'+str(qp)+'.yuv',"success. Time is %.4f"%(end-start))
            qpIdx+=1
        seqIdx+=1
    ...
    if metric['psnr']:
        np.savetxt(result_dir+opt.testingdata_name+'_result_psnr.txt', totalResult_PSNR, fmt = '%.5f')
        np.savetxt(result_dir+opt.testingdata_name+'_result_y_psnr.txt', totalResult_Y_PSNR, fmt = '%.5f')
        np.savetxt(result_dir+opt.testingdata_name+'_result_u_psnr.txt', totalResult_U_PSNR, fmt = '%.5f')
        np.savetxt(result_dir+opt.testingdata_name+'_result_v_psnr.txt', totalResult_V_PSNR, fmt = '%.5f')
        
    if metric['ssim']:
        np.savetxt(result_dir+opt.testingdata_name+'_result_ssim.txt', totalResult_SSIM, fmt = '%.5f')
        np.savetxt(result_dir+opt.testingdata_name+'_result_y_ssim.txt', totalResult_Y_SSIM, fmt = '%.5f')
        np.savetxt(result_dir+opt.testingdata_name+'_result_u_ssim.txt', totalResult_U_SSIM, fmt = '%.5f')
        np.savetxt(result_dir+opt.testingdata_name+'_result_v_ssim.txt', totalResult_V_SSIM, fmt = '%.5f')
