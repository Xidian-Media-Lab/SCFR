    ...
    for qpIdx, QP in enumerate(qplist):  
        ...
        for frame_idx in range(0, frames):            
            ...
            if frame_idx in [0]:      # I-frame bitstream 
                ...
                Iframepath = input_bin_file_path+'/enc/'+opt.testingdata_name+'_'+str(seq)+'_256x256_25_8bit_444_QP'+str(QP)+'/'+'frame'+frame_idx_str+'.bin'
                overall_bits=overall_bits+os.path.getsize(Iframepath)*8 
                
            else:  ## Feature bitstream
                
                interpath=input_bin_file_path+'/kp/'+opt.testingdata_name+'_'+str(seq)+'_256x256_25_8bit_444_QP'+str(QP)+'/'+'frame'+frame_idx_str+'.bin'
...
np.savetxt(save_path+opt.testingdata_name+'_'+'resultBit.txt', totalResult, fmt = '%.5f')
