...
if __name__ == "__main__":
    ...
    parser.add_argument("--original_seq", default='/output.rgb', type=str, help="path to the input testing sequence")
    ...
    ## SCFR
    SCFR_checkpoint_path='SCFR-checkpoint.pth.tar'
    SCFR_config_path='vox-256.yaml'
    SCFR_Analysis_Model, SCFR_Synthesis_Model, cross_scale_m_fusion, Depth_Net, videocompressor, generator_Refine = load_checkpoints(
        SCFR_config_path, SCFR_checkpoint_path, cpu=False)
    modeldir = 'SCFR'
    ...
    for frame_idx in range(0, frames):            
        ...
        if frame_idx in [0]:      # I-frame                      
            ...
            with torch.no_grad(): 
                ...
                kp_reference = SCFR_Analysis_Model(reference, reference_depth['disp', 0]) ################
            ...
            kp_integer=listformat_adptive_CFTE(kp_previous, kp_difference_dec, 1, 2)  #####
            ...
            M_i = cross_scale_m_fusion(kp_current['value'])
            _, kp_current = videocompressor(kp_current, kp_reference)
            prediction = make_prediction(reference, kp_reference, kp_current, SCFR_Synthesis_Model, M_i, generator_Refine) #######################
            gene_end = time.time()
