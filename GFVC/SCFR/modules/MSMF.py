...
from GFVC.SSF.modules.util import DHFE, DHFE_DownBlock2d, DHFE_UpBlock2d
from GFVC.SSF.modules.GDN import GDN
import GFVC.SSF.depth as depth
...
class CPP(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv4(x)
        # Init
        x_init = F.conv_transpose2d(x, self.A, stride=self.block_size)
        ...
        for i in range(self.depth):
            xk, yk, x1, x2, x3, x0 = self.iters[i](self.A, xk, yk, x, x1, x2, x3, x0, i)
            xks.append(xk)
            xks.append(x0)
...
@register_model
def cff4(ratio, pretrained=False, **kwargs):
